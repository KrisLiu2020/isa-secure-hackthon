...
const server = http.createServer((req, res) => {
  ...
  res.setHeader('Access-Control-Allow-Origin', 'https://coruscating-cobbler-0c4041.netlify.app');// Adjust the port if necessary
  ...
  if (req.url === '/login' && req.method === 'POST') {
    ...
    if (cookie && cookie.includes('token=123456')) {
      ...
      res.end(JSON.stringify({ message: 'You are logged in, treasure: PINEAPPLE.' }));
    } else {
      res.writeHead(401, { 'Content-Type': 'application/json' });
